...
export type AskAnswerDecision =
  ...
  | { kind: 'submit' } // Cmd/Ctrl+Enter, 以及 Windows/Linux 的 Alt+Enter: 抹掉 Shift 后放行, 由卡片处理器继续/提交
 ...
 * @returns 处理结果.
 */
export function decideAskAnswerKey(event: KeyEventLike, mode: SendMode, altAsSend = false): AskAnswerDecision {
  ...
  // Windows/Linux 的 Alt+Enter 走同一条继续/提交.
  if (event.metaKey || event.ctrlKey || (altAsSend && event.altKey)) return { kind: 'submit' }
